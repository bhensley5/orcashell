...
/// OSC 133 semantic prompt commands.
///
/// These commands are emitted by shell integration scripts to mark
/// regions of terminal output: prompts, user input, and command output.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SemanticPromptCommand {
    /// `\x1b]133;A\x07` — Prompt region starts.
    PromptStart,
    /// `\x1b]133;B\x07` — User input (command) starts (prompt ends).
    CommandStart,
    /// `\x1b]133;C\x07` — Command has been executed, output follows.
    CommandExecuted,
    /// `\x1b]133;D;exitcode\x07` — Command finished with optional exit code.
...
pub trait Handler {
    ...
    /// Report terminal version (XTVERSION, CSI > q).
    ...
    /// Report a semantic prompt command via OSC 133.
    fn set_semantic_prompt(&mut self, _command: SemanticPromptCommand) {}

    /// Desktop notification via OSC 9, OSC 777, or OSC 99 (finalized).
    fn notify(&mut self, _title: String, _body: String) {}

    /// OSC 99 (Kitty): accumulate a pending notification part.
    fn notify_pending(&mut self, _title: Option<String>, _body: Option<String>) {}

    /// OSC 99 (Kitty): finalize and emit the pending notification.
    fn notify_finalize(&mut self) {}
}
    ...
    fn osc_dispatch(&mut self, params: &[&[u8]], bell_terminated: bool) {
        ...
        match params[0] {
            ...
            b"133" if params.len() >= 2 => {
                match params[1] {
                    b"A" => self.handler.set_semantic_prompt(SemanticPromptCommand::PromptStart),
                    b"B" => self.handler.set_semantic_prompt(SemanticPromptCommand::CommandStart),
                    b"C" => {
                        self.handler.set_semantic_prompt(SemanticPromptCommand::CommandExecuted)
                    },
                    b"D" => {
                        let exit_code = params.get(2).and_then(|p| {
                            core::str::from_utf8(p).ok().and_then(|s| s.parse::<i32>().ok())
                        });
                        self.handler.set_semantic_prompt(
            ...
            // OSC 9: desktop notification (iTerm2 / ConEmu).
            // Format: OSC 9 ; <body> ST
            // Guard: skip OSC 9;4;... (iTerm2 progress bar).
            b"9" if params.len() >= 2 && params[1] != b"4" =>
            {
                let body = params[1..]
                    .iter()
                    .flat_map(|x| str::from_utf8(x))
                    ...
                    .to_owned();
                if !body.is_empty() {
                    self.handler.notify(String::new(), body);
                }
            },

            // OSC 777: notify-osd / terminal-notifier.
            ...
            // Multi-part: params[1] contains colon-separated key=value pairs,
            // params[2] is the payload. Parts arrive in separate dispatches:
            //   p=title → set title, p=body → set body, d=1 → finalize.
            b"99" if params.len() >= 2 => {
                let meta = str::from_utf8(params[1]).unwrap_or("");
                let payload = params
                    .get(2)
                    .and_then(|p| str::from_utf8(p).ok())
                    .unwrap_or("")
                ...
                if is_title {
                    self.handler
                        .notify_pending(Some(payload), None);
                } else if is_body {
                    self.handler
                        .notify_pending(None, Some(payload));
                }

                if is_done {
                    self.handler.notify_finalize();
                }
            },
...
mod tests {
    ...
    struct Osc133Handler {
        commands: Vec<SemanticPromptCommand>,
    }

    impl Handler for Osc133Handler {
        fn set_semantic_prompt(&mut self, command: SemanticPromptCommand) {
            self.commands.push(command);
        }
    }

    fn parse_osc133(bytes: &[u8]) -> Vec<SemanticPromptCommand> {
        let mut parser = Processor::<TestSyncHandler>::new();
    ...
    fn osc133_prompt_start() {
        let cmds = parse_osc133(b"\x1b]133;A\x07");
        assert_eq!(cmds, vec![SemanticPromptCommand::PromptStart]);
    }

    #[test]
    fn osc133_command_start() {
        let cmds = parse_osc133(b"\x1b]133;B\x07");
        assert_eq!(cmds, vec![SemanticPromptCommand::CommandStart]);
    }

    #[test]
    fn osc133_command_executed() {
        let cmds = parse_osc133(b"\x1b]133;C\x07");
        assert_eq!(cmds, vec![SemanticPromptCommand::CommandExecuted]);
    }

    #[test]
    fn osc133_command_finished_no_exit_code() {
        let cmds = parse_osc133(b"\x1b]133;D\x07");
        assert_eq!(cmds, vec![SemanticPromptCommand::CommandFinished { exit_code: None }]);
    }

    #[test]
    fn osc133_command_finished_exit_zero() {
        let cmds = parse_osc133(b"\x1b]133;D;0\x07");
        assert_eq!(cmds, vec![SemanticPromptCommand::CommandFinished { exit_code: Some(0) }]);
    }

    #[test]
    fn osc133_command_finished_exit_127() {
        let cmds = parse_osc133(b"\x1b]133;D;127\x07");
        assert_eq!(cmds, vec![SemanticPromptCommand::CommandFinished { exit_code: Some(127) }]);
    }

    #[test]
    fn osc133_unknown_subcommand_ignored() {
        let cmds = parse_osc133(b"\x1b]133;X\x07");
    ...
    fn osc133_st_terminated() {
        ...
        let cmds = parse_osc133(b"\x1b]133;A\x1b\\");
        assert_eq!(cmds, vec![SemanticPromptCommand::PromptStart]);
    }
