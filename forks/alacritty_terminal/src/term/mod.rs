...
use crate::vte::ansi::{
    ...
    SemanticPromptCommand, StandardCharset,
};
...
pub struct Term<T> {
    ...
    /// Pending OSC 99 (Kitty) notification parts.
...
impl<T: EventListener> Handler for Term<T> {
    ...
    fn report_version(&mut self) {
        ...
        // DCS > | OrcaShell(version) ST
        let version = env!("CARGO_PKG_VERSION");
        let text = format!("\x1bP>|OrcaShell({version})\x1b\\");
    ...
    fn set_semantic_prompt(&mut self, command: SemanticPromptCommand) {
        trace!("SemanticPrompt({command:?})");
        self.event_proxy.send_event(Event::SemanticPrompt(command));
    }
    ...
    #[inline]
    fn notify_pending(&mut self, title: Option<String>, body: Option<String>) {
        let pending = self
    ...
    fn notify_finalize(&mut self) {
        if let Some((title, body)) = self.pending_notification.take() {
            self.event_proxy
                .send_event(Event::Notification { title, body });
        }
    }
