...
use gpui::{
    point, px, quad, transparent_black, Bounds, Edges, Hsla, PathBuilder, Pixels, Point, Size,
    Window,
};
...
/// Returns true if the character is a block element (U+2580-U+259F).
///
/// These are rendered programmatically to guarantee they fill the cell
/// exactly, eliminating hairline gaps that occur with font glyphs.
#[inline]
...
/// Draw a block element character by filling the appropriate fraction of the cell.
///
/// Supports:
/// - Full block (█), shade characters (░ ▒ ▓)
/// - Half blocks (▀ ▄ ▌ ▐)
/// - Quadrant blocks (▖ ▗ ▘ ▙ ▚ ▛ ▜ ▝ ▞ ▟)
/// - Eighth blocks (▁ ▂ ▃ ▅ ▆ ▇ ▔, ▏ ▎ ▍ ▋ ▊ ▉ ▕)
pub fn draw_block_element(ch: char, bounds: Bounds<Pixels>, color: Hsla, window: &mut Window) {
    ...
    match ch {
        // Full block
        '█' => fill_rect(x, y, w, h, color, window),

        // Upper half
        '▀' => fill_rect(x, y, w, h * 0.5, color, window),
        // Lower half
        '▄' => fill_rect(x, y + h * 0.5, w, h * 0.5, color, window),
        // Left half
        '▌' => fill_rect(x, y, w * 0.5, h, color, window),
        // Right half
        '▐' => fill_rect(x + w * 0.5, y, w * 0.5, h, color, window),

        // Lower eighths (▁ through ▇)
        '▁' => fill_rect(x, y + h * 7.0 / 8.0, w, h / 8.0, color, window),
        '▂' => fill_rect(x, y + h * 6.0 / 8.0, w, h * 2.0 / 8.0, color, window),
        '▃' => fill_rect(x, y + h * 5.0 / 8.0, w, h * 3.0 / 8.0, color, window),
        // ▄ handled above as lower half
        '▅' => fill_rect(x, y + h * 3.0 / 8.0, w, h * 5.0 / 8.0, color, window),
        '▆' => fill_rect(x, y + h * 2.0 / 8.0, w, h * 6.0 / 8.0, color, window),
        '▇' => fill_rect(x, y + h / 8.0, w, h * 7.0 / 8.0, color, window),

        // Left eighths (▏ through ▉)
        '▏' => fill_rect(x, y, w / 8.0, h, color, window),
        '▎' => fill_rect(x, y, w * 2.0 / 8.0, h, color, window),
        '▍' => fill_rect(x, y, w * 3.0 / 8.0, h, color, window),
        // ▌ handled above as left half
        '▋' => fill_rect(x, y, w * 5.0 / 8.0, h, color, window),
        '▊' => fill_rect(x, y, w * 6.0 / 8.0, h, color, window),
        '▉' => fill_rect(x, y, w * 7.0 / 8.0, h, color, window),

        // Upper one eighth
        '▔' => fill_rect(x, y, w, h / 8.0, color, window),
        // Right one eighth
        '▕' => fill_rect(x + w * 7.0 / 8.0, y, w / 8.0, h, color, window),

        // Shade characters
        '░' => fill_rect(x, y, w, h, with_alpha(color, 0.25), window),
        '▒' => fill_rect(x, y, w, h, with_alpha(color, 0.5), window),
        '▓' => fill_rect(x, y, w, h, with_alpha(color, 0.75), window),

        // Quadrants
        '▖' => fill_rect(x, y + h * 0.5, w * 0.5, h * 0.5, color, window), // lower left
        '▗' => fill_rect(x + w * 0.5, y + h * 0.5, w * 0.5, h * 0.5, color, window), // lower right
        '▘' => fill_rect(x, y, w * 0.5, h * 0.5, color, window),           // upper left
        '▝' => fill_rect(x + w * 0.5, y, w * 0.5, h * 0.5, color, window), // upper right

        // Quadrant composites (multiple quadrants filled)
        '▙' => {
            // upper left + lower left + lower right
            fill_rect(x, y, w * 0.5, h, color, window);
            fill_rect(x + w * 0.5, y + h * 0.5, w * 0.5, h * 0.5, color, window);
        }
        '▚' => {
            // upper left + lower right (diagonal)
            fill_rect(x, y, w * 0.5, h * 0.5, color, window);
            fill_rect(x + w * 0.5, y + h * 0.5, w * 0.5, h * 0.5, color, window);
        }
        '▛' => {
            // upper left + upper right + lower left
            fill_rect(x, y, w, h * 0.5, color, window);
            fill_rect(x, y + h * 0.5, w * 0.5, h * 0.5, color, window);
        }
        '▜' => {
        ...
        '▞' => {
            ...
            fill_rect(x + w * 0.5, y, w * 0.5, h * 0.5, color, window);
            fill_rect(x, y + h * 0.5, w * 0.5, h * 0.5, color, window);
        }
        '▟' => {
        ...
        _ => {} // Unknown block element. Skip
    }
}

/// Fill a rectangle with a solid color using GPUI's quad primitive.
fn fill_rect(x: Pixels, y: Pixels, w: Pixels, h: Pixels, color: Hsla, window: &mut Window) {
...
/// Return a copy of the color with a modified alpha (multiplied with existing alpha).
