...
| **CLI Parsing** | `clap` | crates.io (v4) | Derive-based CLI for `orcash` binary. |
...
## Explicitly Not Used (Current Product)
...
| **warp / axum / hyper** | No HTTP server in the current desktop product. Local IPC uses Unix domain sockets on macOS/Linux and named pipes on Windows. |
...
│   │       └── socket_server.rs  # Local IPC server for orcash CLI
│   │
...
│   │
│   └── orcashell-cli/                 # (bin) `orcash` command-line interface
│       └── src/
│           ├── main.rs
...
- IPC via named pipe (`\\.\pipe\orcashell-<user-sid>`)
