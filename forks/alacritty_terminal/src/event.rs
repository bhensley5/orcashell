...
use crate::vte::ansi::{Rgb, SemanticPromptCommand};
...
pub enum Event {
    ...
    /// Semantic prompt command received (OSC 133).
    SemanticPrompt(SemanticPromptCommand),

    /// Desktop notification request (OSC 9 / OSC 777).
...
impl Debug for Event {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Event::SemanticPrompt(cmd) => write!(f, "SemanticPrompt({cmd:?})"),
